...
- [ ] Reveal секций по скроллу: `.reveal` (opacity 0 + translateY(26px) → видимое,
...
- [ ] Hero-интро после прелоадера: split заголовка на слова (маска overflow,
...
- [ ] Shine sweep на `.btn` (primary): ::after-блик, transform-only, на hover один
...
### B. 3D-штрихи
- [ ] Тилт бриллианта к курсору: rotation.x/z лерп к cursor*±0.06 rad, вес = uProgress
      (в пыли не мешает комете), поверх покадрового rotation.y.
- [ ] «Дыхание»: uBreath = 1 + 0.015·sin(t·1.6) — масштаб только diamond-таргета
...
### C. Техкачество
- [ ] Self-host шрифтов: woff2-сабсеты (latin+cyrillic) только используемых весов
...
- [ ] Адаптивный DPR: замер среднего кадра (окно ~60 кадров) после старта; если
...
- [ ] Зерно + виньетка: fixed-оверлей (SVG-шум из постера, opacity ~.06, overlay) +
      радиальная виньетка; static, без анимации; pointer-events none; z между
      сценой и контентом.
- [ ] Скролл-прогресс: 2px золотая полоска под шапкой, rAF-троттлинг, transform-only.
- [ ] Пыльца акта 3: uShed (GSAP в окне акта 3), ~7% частиц (aPhase>0.93) — сток вниз
